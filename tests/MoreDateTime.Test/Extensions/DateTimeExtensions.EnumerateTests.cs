namespace MoreDateTime.Tests.Extensions
{
	...
	public partial class DateTimeExtensionsTests
	{
		...
		public void CanCall_EnumerateInStepsUntil_WithEvaluator_10minuteSteps()
		{
			// Arrange
			var @from = _startDate;
			var to = _startDate.AddDays(2);
			var distance = new TimeSpan(DateTimeExtensions.TicksPerMinute * 10);

			// Act
			var result = from.EnumerateInStepsUntil(to, distance, SkipFullHour);

			// Assert
			result.Count().ShouldBe(5 * 24 * 3); // 5*per hour * 3 days
			...
			bool SkipFullHour(DateTime arg)
			{
				return arg.Minute != 0;
			}
		}
		...
		public void CanCall_EnumerateInStepsUntil_WithEvaluatorFalse()
		{
			...
			Func<DateTime, bool> evaluator = x => false;
		...
		public void CanCall_EnumerateInStepsUntil_WithEvaluatorTrue()
		{
			// Arrange
			var distance = TimeSpan.FromDays(1);
			Func<DateTime, bool> evaluator = x => true;
		...
		/// </summary>
		[TestMethod]
		public void CanCall_EnumerateInStepsUntil2()
		{
			// Arrange
			var @from = _startDate;
			var to = _startDate.AddDays(10);
		...
		{
			Should.Throw<ArgumentException>(() => _startDate.EnumerateInStepsUntil(_startDate.AddSeconds(1), TimeSpan.FromSeconds(10)).Count());
		}
